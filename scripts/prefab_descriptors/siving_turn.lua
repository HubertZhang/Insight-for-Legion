...
local function Describe(inst, context)
    ...
    if cpt then
        if cpt.timedata and cpt.timedata.all and cpt.timedata.pass then
            local description = string.format(tr.turn_description, context.time:SimpleProcess(cpt.timedata.all - cpt.timedata.pass))
        ...
        if held_item and held_item.prefab == "siving_rock" then
            if cpt.energytime < cpt.energytime_max then
                ...
                if held_item.components and held_item.components.stackable ~= nil then
                    ...
                    descriptions[#descriptions + 1] = { name = "siving_turn_charge", priority = 0, description = description }
                end
            end
        end
    end
